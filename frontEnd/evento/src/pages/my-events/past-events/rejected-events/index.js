...
function PastRejectedEvents(props) {
    const {id} = useParams()
    ...
    // useEffect(() => {
    //     axios
    //         .get(`http://localhost:4000/invitations/response/Completed/No/${id}`)
    //         .then(response => {
    ...
    // }, [])
      ...
      useEffect(async () => {
        try{
          try {
            const x = await apiHandler('get',`invitations/response/Completed/No/${id}`);
            //console.log(x.data);
            setEvents(x.data);
        } catch (error) {
            const x = await tokenHandler('get',`invitations/response/Completed/No/${id}`,sessionStorage.getItem('refreshToken'),apiHandler);
            setEvents(x.data);
        }
        }
        catch{
    ...
    return (
        ...
        <div className="upcomingEventsTable">
            ...
            <MyEventsTable
                ...
                // onClick ={mapping}
                myEventType={services.myEventType.COMPLETED_EVENT.REJECTED_EVENT}
            />
        </div>
        </>
    )
...
export default PastRejectedEvents;