...
function InvitedEvents() {
    ...
    useEffect(() => {
        axios
            .get(`http://localhost:4000/invitations/response/NotResponded/${id}`)
            .then(response => {
...
//   ))}
    ...
    return (
        <div className="upcomingEventsTable">
            <SizedBox height="2vh" />
            <MyEventsTable
                titles={['Event ID','Event-Titles', 'Start Date','End Date','Status','Actions']}
                events={events}
                // onClick={navigateToEvent(events.id)}
                // onClick = {() => navigate(`../view-event/${id}/${events.id}`)}
                // onClick ={mapping}
                eventType={services.myEventType.UPCOMING_EVENT.INVITEDEVENT}
            />
        </div>
    )
}
