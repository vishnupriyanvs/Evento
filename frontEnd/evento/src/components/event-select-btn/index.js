...
function StatusSelectionBtn(props) {
  ...
  const styleSet = (item, key) => {
    ...
    if (item === "Cancelled")
      a[key].setAttribute(
        "style",
        "outline: 1px solid rgb(243, 20, 20); color:rgb(243, 20, 20);"
      );
    if (item === "Active")
      a[key].setAttribute(
        "style",
        "outline: 1px solid #0000FF; color: #0000FF"
      );
    if (item === "Completed")
      a[key].setAttribute(
        "style",
        "outline:1px solid rgb(20, 243, 20); color: rgb(20, 243, 20)"
      );
  };
    ...
    try {
      try {
        const response = await apiHandler(
          "put",
          `events/cancellation/reason/${props.eventid}`,
          {
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        if (getComputedStyle(x[props.index]).display === "none")
          x[props.index].setAttribute(
            "style",
            "display:flex; visibility: visible; flex-direction:column; width: 100%"
          );
        else
          x[props.index].setAttribute(
            "style",
    ...
    >
      <div className="SelectnBtn1-content">
        {defaults === "Cancelled" &&
        options.includes(defaults) &&
        (props.eventType === services.eventType.UPCOMING_EVENT ||
          props.eventType === services.eventType.ONGOING_EVENT) ? (
          <Popup
            trigger={
              <div id="select-btn-value">
                {props.reason !== undefined ? "Reason" : defaults}
              </div>
            }
              ...
              <Form onSubmit={(e) => e.preventDefault()}>
                ...
                {!flag ? (
                  <Button
                    variant="primary"
                    type="submit"
                    onClick={() => handleEventCancellation(props.invitationId)}
                  >
                    Submit
                  </Button>
                ) : (
                  <Button
                    variant="danger"
                    onClick={() => {
                      const x = document.getElementById("triggerBox");
                      console.log(x);
      ...
      {props.role === "Admin" ? (
        <div className="SelectnBtn1-options">
          {options.map((item, i) => {
            if (item !== defaults) {
              return (
                <div
                  key={i}
                  className="SelectnBtn1-option"
                  onClick={async () => {
                    setDefault(item);
                    let a = document.querySelectorAll(".SelectnBtn1-content");
                    if (item === "InProgress") {
                      ...
                      if (result) {
                        a[props.index].setAttribute(
                          "style",
                          "outline:1px solid #FF8A00; color:#FF8A00"
                        );
                        handleChange(item);
                      }
                      window.location.reload(false);
