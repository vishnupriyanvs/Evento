    ...
    switch (page) {
      case services.eventType.ONGOING_EVENT:
        ...
        setTrow(
          tRow.length > (i + 1) * 10
            ? tRow
                .slice(tRow.length - (i + 1) * 10, tRow.length - i * 10)
                .reverse()
            : tRow.slice(0, tRow.length - i * 10).reverse()
        );
        break;
  ...
  return (
    <div className="center-elements">
      ...
      <div className="pageList-items">
        ...
        {pages.map((item, i) => (
          <span onClick={() => setCounter(item)}>{item + 1}</span>
        ))}
        {counter <=
        (props.events.length % 10 !== 0
          ? Math.floor(props.events.length / 10) + 1 - 1
          : Math.floor(props.events.length / 10) - 1) -
          1 ? (
          <button
            onClick={() => {
              setCounter(counter + 1);
